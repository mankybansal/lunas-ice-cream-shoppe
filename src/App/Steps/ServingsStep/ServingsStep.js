...
class ServingsList extends React.Component {
  ...
  selectServing(item) {
    ...
    Order.CurrentItem.Flavors = Order.CurrentItem.Flavors.slice(
      0,
      Order.CurrentItem.Serving.scoops
    );
    Order.CurrentItem.Toppings = Order.CurrentItem.Toppings.slice(
      0,
      Order.CurrentItem.Serving.toppings
    );
  ...
  render() {
    const listItems = this.props.Servings.map((Serving) => {
      ...
      if (
        ...
        this.props.Order.CurrentItem.Serving.id === Serving.id
      )
      ...
      return (
        <div
          ...
          onClick={() => this.selectServing(Serving)}
        >
...
class ServingsStep extends React.Component {
  ...
  stepHandler(gotoStep) {
    ...
    if (this.props.Order.CurrentItem.Serving) {
      ...
      this.props.stepHandler(gotoStep);
    } else alert("Please select a serving size");
  }

  render() {
    if (this.props.currentStep !== AppConfig.steps.Servings) return null;
    ...
    return (
      <div className="App-header-padding">
        ...
        <div
          className="Button-step Button-next Button-step-full"
